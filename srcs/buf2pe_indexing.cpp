...
// 暂时在这个文件上,研究层的不同的访存模式,indexing正确之后,再合并
...
void init_index_table(address_t_8 index_table[(Noy / Poy) * (Nox / Pox) * Nkx * Nky * (Pox + 1) * Poy])
{
    int table_index = 0;
...
#pragma HLS STREAM variable = inner_fifos depth = 6 dim = 1
    // inner fifos
    address_t_8 index_table[Noy / Poy * Nox / Pox * Nkx * Nky * (Pox + 1) * Poy];
...
#pragma region concurrent
                CONCURRENT:
                    for (int dsp_y = 0; dsp_y < Poy; dsp_y++)
                    {
                        ...
                        {
                            bool part1_2 = (((kernel_x == 0) || (dsp_x == Pox)) && ((dsp_y == Poy - 1) || (kernel_y == 0)));
                            bool part3 = (kernel_x != 0) && (dsp_x != Pox) && ((dsp_y == Poy - 1) || (kernel_y == 0));
                            bool part4 = (dsp_y != Poy - 1) && (kernel_y != 0) && ((dsp_x != Pox));
                            int input_buffer_index = index_table[table_index] + index_offset;
                            if (part1_2)
                            {
                                bool west_part_1 = (kernel_x == 0) && (dsp_x == 0) && ((dsp_y == Poy - 1) || (kernel_y == 0));
                                bool east_part_1 = ((kernel_x == Nkx - 1) && (dsp_x == Pox)) && ((dsp_y == Poy - 1) || (kernel_y == 0));
                                if ((output_x == 0 && west_part_1) || (output_x == Nox / Pox - 1 && east_part_1))
                                    input_registers[dsp_y][dsp_x] = -1;
                                else
