...
extern "C" void apatb_top_hw(volatile void * __xlx_apatb_param_src, volatile void * __xlx_apatb_param_dst) {
  ...
  vector<int> __xlx_src_tmp_vec;
  while (!((hls::stream<int>*)__xlx_apatb_param_src)->empty() && __xlx_src_V_tmp_Count < __xlx_src_V_read_Size) {
    __xlx_src_tmp_vec.push_back(((hls::stream<int>*)__xlx_apatb_param_src)->read());
    __xlx_src_V_tmp_Count++;
  }
  ap_apatb_src_V_cap_bc = __xlx_src_tmp_vec.size();
  // store input buffer
  int* __xlx_src_input_buffer= new int[__xlx_src_tmp_vec.size()];
  ...
  int* __xlx_dst_input_buffer= new int[ap_apatb_dst_V_cap_bc];
  // DUT call
  top(__xlx_src_input_buffer, __xlx_dst_input_buffer);
  for (unsigned i = 0; i <ap_apatb_dst_V_cap_bc; ++i)
    ((hls::stream<int>*)__xlx_apatb_param_dst)->write(__xlx_dst_input_buffer[i]);
}
